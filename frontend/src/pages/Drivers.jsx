...
const Drivers = () => {
  ...
  const [form, setForm] = useState({ name: '', license_number: '', phone: '', status: 'active' });
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      setShowForm(false);
      setForm({ name: '', license_number: '', phone: '', status: 'active' });
      fetchDrivers();
                ...
                <tr key={d.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm">
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                      ...
                      d.status === 'inactive' ? 'bg-blue-100 text-blue-800' :
